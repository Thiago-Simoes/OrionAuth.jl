...
if HAS_OXYGEN
    ...
    function setupOxygenRoutes()
        ...
        Oxygen.@get "/protected" function(req::HTTP.Request)
            try
                ...
                return HTTP.Response(200, ["Content-Type" => "text/plain"], "protected content")
            catch ex
    ...
    @testset verbose=true "Oxygen Framework Integration (Auth only)" begin
        ...
        user, signupData = signup("oxygen.route@thiago.com", "Oxygen Route Tester", "testPwd")
        ...
        resp = HTTP.request("GET", base * "/protected";
        ...
        resp = HTTP.request("GET", base * "/user";
            headers = ["Authorization" => "Bearer $initialToken"])
        @test resp.status == 200
        ...
        resp = HTTP.request("DELETE", base * "/delete-resource";
